...
{
    public class BitFrameworksWebApiHttp
    {
        ...
        public BitFrameworksWebApiHttp(string Url, object AutoCreateOption, string Token, string DataStoreId)
        ...
        //     "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>",
        //     typeof(BitFrameworksWebApi))]
        public BitFrameworksWebApiHttp(string Url, string WebApicontroller, string LoginController, object AutoCreateOption, string Token, string DataStoreId)
        {
        ...
        public async Task<T> ModifyData<T>(byte[] dmlStatements)
        {
            ...
            var Data= await  PostStreamAsync(dmlStatements, new CancellationToken(false), $"{_WebApiController}/ModifyData");
            return GetObjectsFromByteArray<T>(Data);
        ...
        private  async Task<Byte[]> PostStreamAsync(object content, CancellationToken cancellationToken,string Url)
        {
            using (var client = new HttpClient())
            //using (var request = new HttpRequestMessage(HttpMethod.Post, Url))
            using (var request = PrepareRequest(Url))
        ...
        private HttpRequestMessage PrepareRequest(string Url)
        {
            ...
            requestMessage.Headers.Add("AutoCreateOptions", _AutoCreateOption.ToString());
            requestMessage.Headers.Add("DataStoreId", _DataStoreId);
        ...
        public async Task<T> SelectData<T>(byte[] selects)
        {
            ...
            //}
            //throw ThrowFormatedException(response);
            var Data = await PostStreamAsync(selects, new CancellationToken(false), $"{_WebApiController}/SelectData");
            return GetObjectsFromByteArray<T>(Data);
        ...
        public async Task<T> UpdateSchema<T>(bool dontCreateIfFirstTableNotExist, string tables) where T : new()
        {
            ...
            var Data = await PostStreamAsync(value, new CancellationToken(false), $"{_WebApiController}/UpdateSchema");
            return GetObjectsFromByteArray<T>(Data);


            //throw ThrowFormatedException(response);
        }
        public async Task<object> Do(string Command, object Args)
        {
            ...
            var Data = await PostStreamAsync(ToByteArray(output), new CancellationToken(false), $"{_WebApiController}/Do");
            return GetObjectsFromByteArray<object>(Data);
